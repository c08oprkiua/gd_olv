...
func parse_for_communities():
	...
	var ret_array:Array[CommRes]
	var current_comm:CommRes
	while read() != ERR_FILE_EOF:
		if ((get_node_type() == XMLParser.NODE_ELEMENT) and (get_node_name() == "communities")):
			current_comm = parse_for_community()
			ret_array.append(current_comm)
